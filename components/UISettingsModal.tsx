...
export default function UISettingsModal({ onClose }: { onClose: () => void }) {
  ...
  const { gameSettings, updateGameSettings } = useStore()
  ...
  return (
    <AnimatePresence>
      <div key="ui-settings-modal" className="fixed inset-0 z-50 flex items-center justify-center p-4">
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
        ...
        >
          <Card elevate className="p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white">{t('uiSettings')}</h2>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                <Toggle
                  ...
                  onChange={(checked) => updateGameSettings({ showHighestLowest: checked })}
                />

                <Toggle
                  label={t('showTotalScore')}
                  checked={gameSettings.showTotalScore}
                  onChange={(checked) => updateGameSettings({ showTotalScore: checked })}
                />
              </div>
